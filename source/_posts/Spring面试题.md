...
常用的就2种，required和required new

required和required new的区别:

1. 使用required，内层事务与外层事务共用一个事务，出现异常同时回滚。

2. 使用required new时，内层事务与外层事务是两个独立的事务。一旦内层事务进行了提交后，外层事务不能对其进行回滚。两个事务互不影响。
