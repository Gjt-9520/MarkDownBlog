...
date: 2024-03-24
description: ""
cover: https://github.com/Gjt-9520/MarkDownBlog/blob/main/source/coverImages/Aimage-135/Aimage107.jpg?raw=true
tags: ["JavaSE","集合"]
category: "笔记"
updated: 2024-03-25
...
private V put(K key, V value, boolean replaceOld) {
    ...
    Entry<K,V> t = root;
    // 判断根结点是否为null
    // 如果为null,表示当前是第一次添加,会把当前要添加的元素,当做根结点
    // 如果不为null,表示当前不是第一次添加,跳过这个判断继续执行下面的代码
    if (t == null) {
        // 方法的底层,会创建一个Entry对象,把他当做根结点
    ...
    if (cpr != null) {
        ...
        do {
            // 把根结点当做当前结点的父结点
            parent = t;
            // 调用compareTo方法,比较根结点和当前要添加结点的大小关系
...
private void fixAfterInsertion(Entry<K,V> x) {
    ...
    // 按照红黑规则进行调整
    // parentOf:获取x的父结点
    // parentOf(parentOf(x)):获取x的祖父结点
    // leftOf:获取左子结点
    while (x != null && x != root && x.parent.color == RED) {
        // 判断当前结点的父结点是祖父结点的左子结点还是右子结点
        // 目的:为了获取当前结点的叔叔结点
        if (parentOf(x) == leftOf(parentOf(parentOf(x)))) {
            // 表示当前结点的父结点是祖父结点的左子结点
            // 那么下面就可以用rightOf获取到当前结点的叔叔结点
            ...
            // 表示当前结点的父结点是祖父结点的右子结点
            // 那么下面就可以用leftOf获取到当前结点的叔叔结点
